...
- TFEX PDF import: open **บันทึกการเทรด TFEX → นำเข้า PDF จาก Pi**, select one or more PDFs (one shared password for all of them), review the rows and choose a broker (or create Pi Securities with the import), then save. Each file accepts Pi text-based daily confirmations up to 10 MB and two pages for S50 quarterly futures (200 THB/point) and USD Futures (1,000 THB/point). Selected files are reconciled together in trading-date order — a close in a later file can match an open lot from an earlier one in the same selection — and saved in a single commit; a file that fails to read or fails validation (wrong password, corrupt file, duplicate document) is skipped with its reason shown and the rest of the selection still imports. Multiple opening cost lots can reconcile to one closing execution; a second continuation/signature page and the outstanding-position table are excluded from the closing ledger. Other brokers, scanned PDFs, options and unsupported contracts are rejected with an explanation. Files and passwords are processed in the browser with bundled PDF.js; neither is uploaded or retained.
...
- There is no historical **valuation** chart; the overview growth panel shows the current valuation only, because there is no historical price series to derive one from. The TFEX journal's cumulative realized P&L chart is a documented exception: it charts a ledger fact (the running sum of already-closed trades' P&L) that is fully known per trade, not an estimate.
- TFEX cumulative P&L chart: on the TFEX page, below the stat cards, a line chart of running realized P&L bucketed by each trade's close date. Three view modes — ทั้งหมด (all-time, default, running from the first-ever closed trade), รายปี (a specific year, resets to 0 at Jan 1), ไตรมาส (a specific quarter, resets to 0 at the quarter's start) — with a dropdown to pick which year/quarter, populated only with periods that have closed trades. The period filter affects only this chart, never the stat cards or the trades table below it.
