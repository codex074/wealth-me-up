...
test('LONG, break-even and open-only statements',async()=>{
 const long=await parsePiTfex(statement.replace('L Close','S Close').replace('0 2 900.00','2 0 900.00').replace('C 2 0 905.00','C 0 2 905.00').replaceAll('-2,000.00','2,000.00'),SALT);assert.equal(long.rows[0].side,'LONG');assert.equal(long.gross,2000);
 const even=await parsePiTfex(statement.replace('900.00','905.00').replaceAll('-2,000.00','0.00'),SALT);assert.equal(even.gross,0);
 const open=await parsePiTfex(statement.replace('L Close','S Open').replace(/POSITION CLOSING[\s\S]*STATEMENT OF ACCOUNT/,'STATEMENT OF ACCOUNT'),SALT);assert.equal(open.rows[0].exit,null);assert.equal(open.rows[0].side,'SHORT');assert.equal(open.gross,0);
});
