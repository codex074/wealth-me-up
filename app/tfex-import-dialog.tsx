...
export function TfexImportDialog({data,save,close}:{data:Portfolio;save:(next:Portfolio)=>Promise<void>;close:()=>void}) {
 ...
 return <Dialog open onOpenChange={open=>{if(!open&&!busy)close();}}><DialogContent showCloseButton={false} className="!max-w-[min(1100px,calc(100%-2rem))] max-h-[90dvh] overflow-y-auto !bg-[#fcfdf9] !rounded-xl">
  <DialogHeader><DialogTitle>นำเข้า TFEX จาก PDF</DialogTitle><DialogDescription>ใบยืนยัน Pi · S50 Futures · PDF ข้อความ 1 หน้า ไม่เกิน 10 MB</DialogDescription></DialogHeader>
  ...
  {!statement?<form onSubmit={read} className="space-y-4">
   ...
   {preview&&<>
    ...
    <Table><TableHeader><TableRow>{["สัญญา / ฝั่ง","จำนวน","เปิด → ปิด","วันที่เปิด → ปิด","ค่าธรรมเนียมใน PDF","ค่าธรรมเนียมเปิดเดิม","กำไรสุทธิ (THB)"].map(label=><TableHead key={label}>{label}</TableHead>)}</TableRow></TableHeader><TableBody>{preview.rows.map(({trade,openingFee,needsFee,matched},index)=><TableRow key={index}>
   ...
   <div className="flex flex-wrap justify-end gap-3"><button className="btn" disabled={busy} onClick={()=>{setStatement(null);setFees({});setError("");setFile(null);}}>เลือกไฟล์ใหม่</button><button className="btn" disabled={busy} onClick={close}>ยกเลิก</button><button className="btn primary" disabled={busy||!preview||preview.needsFees} onClick={()=>void commit()}>{busy?"กำลังบันทึก…":`บันทึก ${statement.rows.length} รายการ`}</button></div>
  </>}
