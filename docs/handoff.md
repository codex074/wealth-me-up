...
## 2026-09-19 — Bank logos on cash accounts; compact account/platform cards (Claude)

**Changed**: Committed pre-existing uncommitted work (bank-logo picker) alongside a card-density fix requested by the owner. `lib/banks.ts` lists 16 Thai banks/providers with a logo path; `public/logos/banks/` holds the matching SVG/PNG assets; `BankLogo` (`app/portfolio-workspace.tsx`) renders the logo with a first-letter fallback on load error, used in the account form's bank picker, the account cards, and the dashboard's recent-accounts row (`app/dashboard.tsx`). Separately, `.cards-grid`/`.detail-card` in `app/globals.css` were too large (24px padding, 27–32px stat numbers, fixed 3-column grid) — switched to `auto-fill, minmax(230px,1fr)` and shrank padding/margins/icon/number sizing so account and platform cards read as compact, not full-width tiles.

**Verified**: `tsc --noEmit` clean; lint unchanged at the same 4 pre-existing errors plus one new expected `no-img-element` warning (same pattern already accepted for `market-view.tsx`'s `<img>`); all 53 `node --test` cases pass; production build clean. Manually exercised in local dev (`localhost:5173`, synthetic accounts, not the real owner's data): added a KTB account through the bank picker, confirmed the logo renders and the account/platform cards are visually compact at desktop width.

**Deployment**: commit `8d360b6` pushed to `github main`, shipped to pve1 / LXC 103 with `deploy/redeploy.sh`. Post-deploy smoke test passed 7/7; public `/login` returned 200. The owner's live portfolio was not touched.

**Still open (non-blocking)**: the 4 pre-existing lint errors (`react-hooks/set-state-in-effect` ×3, `no-html-link-for-pages` ×1) and the `no-img-element` warnings are unaddressed, same as prior handoff entries.
